...
public partial class KapaliCariRapor : System.Web.UI.Page
{
    SqlConnection conn, connBizim;
    SqlDataAdapter adpVeri;
    DataTable tblVeri;
    string bayi, kullanici, parola,musteriKod;
    int sayi;
    ...
    {
        adpVeri = new SqlDataAdapter("SELECT  [SEÇ]='',CODE AS [CARİ KOD],DEFINITION_ " +
            "AS [CARİ AD],[DURUM]='KAPALI' FROM LG_316_CLCARD WHERE DBSLIMIT3=1", conn);
    ...
    {
        adpVeri = new SqlDataAdapter("SELECT  [SEÇ]='',CODE,DEFINITION_ AS [CARİ AD],[DURUM]='KAPALI' " +
            "FROM LG_316_CLCARD WHERE DBSLIMIT3=1 AND DEFINITION_ LIKE '%" + txtCari.Text + "%'", conn);
        tblVeri = new DataTable();
        adpVeri.Fill(tblVeri);
        this.grdKapatilacak.DataSource = tblVeri;
        this.grdKapatilacak.DataBind();
        this.grdKapatilacak.Columns[1].Visible = false;
        chkOlustur();
    }
    ...
    protected void btnAc_Click(object sender, EventArgs e)
    {
        for (int i = 0; i < grdKapatilacak.Rows.Count; i++)
        {
            if (c.Checked == true)
            {
                ...
                foreach (DataRow item in tblCariPlaka.Rows)
                {
                    ...
                    tts.SETCARDSTATUSRESULT pro = plakaKapama.SetPlateStatus(bayi, kullanici, parola, musteriKod, item[0].ToString(), "1", "10", "11111111111111111111");
                ...
                SqlCommand cmdDurum = new SqlCommand("UPDATE LG_316_CLCARD SET DBSLIMIT3=0 WHERE CODE='" + Session["CariRef"].ToString() + "'", conn);
                conn.Open();
        ...
        Page.ClientScript.RegisterStartupScript(this.GetType(), "Scripts", "<script>alert(Seçili Carilerin Yakıt Alımı Açılmıştır);</script>");
    }
}