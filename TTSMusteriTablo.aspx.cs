...
public partial class TTSMusteriTablo : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                SqlDataAdapter adpSatis = new SqlDataAdapter("SELECT ISIM FROM CRM..KULLANICI WHERE ISIM IS NOT NULL", connBizim);
                DataTable tblSatis = new DataTable();
                adpSatis.Fill(tblSatis);
                for (int i = 0; i < tblSatis.Rows.Count; i++)
                {
                    cmbSatisEleman.Items.Add(tblSatis.Rows[i][0].ToString());
                }
            }
            else
            {
                SqlDataAdapter adpSatis = new SqlDataAdapter("SELECT ISIM FROM CRM..KULLANICI WHERE KULLANICIAD = '" + Session["Kullanici"].ToString() + "'", connBizim);
                DataTable tblSatis = new DataTable();
                adpSatis.Fill(tblSatis);
                for (int i = 0; i < tblSatis.Rows.Count; i++)
                {
            ...
            //cmbSatisEleman.SelectedIndex = 0;
        }
    }
